...
const uint8_t LSM9DS1 = 0x1E; // edit: set to LSM9DS1 sensor
...
//  Serial.print(i2cData[0]);
  ...
  while (i2cWrite(LSM9DS1, 0x19, i2cData, 4, false)); // Write to all four registers at once
  while (i2cWrite(LSM9DS1, 0x6B, 0x01, true)); // PLL with X axis gyroscope reference and disable sleep mode

  while (i2cRead(LSM9DS1, 0x0F, i2cData, 1));
//  if (i2cData[0] != 0x1E) { // Read "WHO_AM_I" register
...
#endif
#if 1 // Set to 1 to print the IMU data
...
void updateLSM9DS1() {
//  while (i2cRead(LSM9DS1, 0x28, i2cData, 14)); // Get accelerometer and gyroscope values
...
//  magX = ((i2cData[14] << 8) | i2cData[15]);
//  magZ = ((i2cData[16] << 8) | i2cData[17]);
...
void calibrateMag() { // Inspired by: https://code.google.com/p/open-headtracker/
  i2cWrite(LSM9DS1, 0x0F, i2cData, 4, false);
  delay(100); // Wait for sensor to get ready
  updateLSM9DS1(); // Read positive bias values

  int16_t magPosOff[3] = { magX, magY, magZ };

  i2cWrite(LSM9DS1, 0x0F, i2cData, 4, false); // Write to all four registers at once
  delay(100); // Wait for sensor to get ready
  updateLSM9DS1(); // Read negative bias values
